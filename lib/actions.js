...
Actions.prototype.logs = function() {
  ...
  for(var os in self.sessionsMap) {
    ...
    sessionsInfo.sessions.forEach(function(session) {
      ...
      session.execute('lsb_release -a 2> /dev/null', {}, function (erro, code, logs) {
        if (/Release:.*1[6|8]\./.test(logs.stdout)) {
          var command = 'sudo tail ' + tailOptions + ' /var/log/syslog | grep ' + self.config.appName;
...
Actions.init = function() {
  var destMupJson = path.resolve('yamup.json');
  ...
  var exampleMupJson = path.resolve(__dirname, '../example/yamup.json');
